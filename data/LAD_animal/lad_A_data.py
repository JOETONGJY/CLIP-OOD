...
class Processed_LADA(Dataset):
    def __init__(self, args, data_root, split, meta_root=None,attr_name=None,src_dm_texts=None,tgt_dm_texts =None):

        if split == "train":
            self.annos = open(os.path.join(meta_root, "train_animal.txt")).readlines()
        elif split == "test":
            self.annos = open(os.path.join(meta_root, "test_animal.txt")).readlines()
        else:
            raise ValueError("split must be train or test")
        with open(r"data/LAD_animal/class_attribute_avg.pkl", "rb") as f:
        ...
        with open(r"data/LAD_animal/class.txt", "r") as f:
        ...
        # with open(r"data/LAD_animal/LAD_animal_concept.txt","r") as f:
        with open(r"data/LAD_animal/LAD_animal_conceptNet_concepts.txt", "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        attr_label = torch.tensor([0] * 240)
        return image, label, attr_label

class Processed_LADAS(Dataset):
    def __init__(self, args, data_root, split, meta_root=None, classname2id=None,concept2id=None):
        ...
        if split == "train":
            ...
            self.annos = open(os.path.join(meta_root, "test_sculpture_animals.txt")).readlines()
        self.data_root = data_root
        device = args.device
...
class Processed_LADA_Labo(Dataset):
    def __init__(self, args, data_root, split, meta_root=None, attr_name=None,src_dm_texts=None,tgt_dm_texts =None):
...
class Processed_LADAS_Labo(Dataset):
    def __init__(self, args, data_root, split, meta_root=None, classname2id=None,concept2id=None):
        ###
        if split == "train":
            raise Exception("No processed data for train split")
...
if __name__ == '__main__':
    ...
    train_dataset = Processed_LADA(args,data_root="/mnt/20ee3a83-3f42-40a7-b9b4-cfc9beade036/zzq/DomainData/LAD_data/animals",
                                          split="train",
    ...
    with open("data/LAD_animal/class_avg_attribute.pkl", "wb") as f:
